...
def sqrt(n, one):
    """
    Calculate the square root of an integer, with the precision passed in as one.
...
def pi_chudnovksky(one):
    """
    Calculate pi using a Chudnovksky's series.
...
def validate_nonnegative_interger():
    """
    Ask the user for input and only return when a nonnegative integer under 10000 is given.
...
def main():
    digits = validate_nonnegative_interger()
    pi = str(pi_chudnovksky(10**(digits * 10)))[:digits]
    print (pi [0] + "." + pi[1:])
